...
describe("getEventAnalytics", () => {
  ...
  it("should return analytics correctly for valid event", async () => {
    mockReq = { params: { id: "5" } };

    const mockQuery = jest
      .fn()
      // First query: tickets sold from ClaimedTickets
      .mockResolvedValueOnce([[{ ticketsSold: 10 }], []])
      // Second query: attendance from TicketPasses
      .mockResolvedValueOnce([[{ attendance: 8 }], []])
      // Third query: event info (price + capacity)
